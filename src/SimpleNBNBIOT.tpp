...
#ifndef SRC_SIMPLE_NB_NBIOT_H_
#define SRC_SIMPLE_NB_NBIOT_H_

#include "SimpleNBCommon.h"

#define SIMPLE_NB_SUPPORT_NBIOT
...
template <class modemType> class SimpleNBNBIOT {
  public:
    /*
     * SIM card functions
     */
    // Unlocks the SIM
    bool simUnlock(const char *pin) { return thisModem().simUnlockImpl(pin); }
    // Gets the CCID of a sim card via AT+CCID
    String getSimCCID() { return thisModem().getSimCCIDImpl(); }
    // Asks for TA Serial Number Identification (IMEI)
    String getIMEI() { return thisModem().getIMEIImpl(); }
    // Asks for International Mobile Subscriber Identity IMSI
    String    getIMSI() { return thisModem().getIMSIImpl(); }
    SimStatus getSimStatus(uint32_t timeout_ms = 10000L) { return thisModem().getSimStatusImpl(timeout_ms); }
  ...
  protected:
    ...
    SimStatus getSimStatusImpl(uint32_t timeout_ms = 10000L)
    {
        for (uint32_t start = millis(); millis() - start < timeout_ms;) {
            ...
            int8_t status = thisModem().waitResponse(GF("READY"), GF("SIM PIN"), GF("SIM PUK"), GF("NOT INSERTED"), GF("NOT READY"));
            thisModem().waitResponse();
            switch (status) {
            case 2:
            case 3:
  ...
  protected:
    ...
    String getOperatorImpl()
    {
        ...
        if (thisModem().waitResponse(GF("+COPS:")) != 1) {
            return "";
        }
